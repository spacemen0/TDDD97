...
class FlaskAppTests(unittest.TestCase):
    ...
    def test_02_sign_up_invalidate_data(self):
        ...
        invalid_emails = [
            f"{invalid_email}@example",
            f"{invalid_email}@.com",
            f"{invalid_email}@exa mple.com",
        ]
        ...
        for password in passwords:
            ...
            try:
                ...
                if password == passwords[0]:
                    self.assertTrue(
                        is_positive, "It looks like the sign in was not successful."
                    )
    ...
    def test_04_change_password(self):
        ...
        combinations_dict_hardcoded = {
            ...
            2: {"old_passwords": "password123", "new_passwords": None, "tokens": token},
            3: {
                "old_passwords": "password123",
                "new_passwords": "password1234",
                "tokens": token,
    ...
    def test_06_get_user_data_by_email(self):
        ...
        combination_dict = {
            0: {"email": self.user_email, "token": token},
            1: {"email": self.user_email, "token": "invalid_token123"},
            2: {"email": self.user_email, "token": None},
            3: {"email": None, "token": token},
            4: {"email": self.user_email_2, "token": token},
            5: {"email": self.random_email, "token": token},
        }
    ...
    def test_07_post_message(self):
        ...
        message_key = (
            input("Enter the message key name in the body (default is 'message'): ")
            or "message"
        )
        combinations_dict_hardcoded = {
            0: {
            ...
            2: {"message": new_input_message, "email": self.user_email, "token": None},
            3: {"message": new_input_message, "email": None, "token": token},
            4: {"message": None, "email": self.user_email, "token": token},
            5: {
                "message": new_input_message,
        ...
        for i in range(len(combination_dict)):
            try:
                ...
                if i == 0:
                    ...
                    self.assertFalse(
                        is_positive,
                        "It looks like the get user messages by email was successful "
                        "with non-existing email.",
                    )
                    print(
